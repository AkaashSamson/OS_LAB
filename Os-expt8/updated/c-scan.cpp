...
int n, req_seq[MAX], current_head, traversal[MAX + 1], ubound, lbound = 0,prev_hd;
char direction;
...
int c_scan(const int req_seq[], int n, int current_head, char direction, int traversal[], int &index)
{
    ...
    if (direction == 'L' || direction == 'l')
    {
        ...
        current_head = ubound;
        traversal[index++] = current_head;
        total_head_movement += move_left(right, right_size, current_head, traversal, index);
    }
    else if (direction == 'R' || direction == 'r')
    {
        ...
        current_head = lbound;
        traversal[index++] = current_head;
        total_head_movement += move_right(left, left_size, current_head, traversal, index);
    }
...
int main()
{
    ...
    cin >> current_head;
    cout << "Enter the previous head:  ";
    cin >> prev_hd;

    if(prev_hd >= current_head)
       direction = 'L';
    else 
       direction = 'R';

    int total_head_movement = c_scan(req_seq, n, current_head, direction, traversal, index);
